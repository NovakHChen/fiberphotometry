...
def create_video_dict(
    ...
    Creates a dictionary with the following keys:
        ...
        'end' : frame at which to end.  set to None if processing whole video [int]
    ...
    Args:
        ...
        dsmpl: proptional degree to which video should be downsampled by (0-1).
        width: proportion by which to stretch frame width [float]
        height: proportion by which to stretch frame height [float]
        ftype: (only if batch processing) video file type extension (e.g. 'wmv') [str]
        FileNames: (only if batch processing) List of filenames of videos in folder to be batch processed.  [list]
        cal_frms: number of frames to calibrate based upon [int]
    -------------------------------------------------------------------------------------
    Returns:
        video_dict: dictionary with the following keys:
            'dpath' : directory containing files [str]
            'file' : filename with extension, e.g. 'myvideo.wmv' [str]
            'fps' : frames per second of video files to be processed [int]
            'start' : frame at which to start. 0-based [int]
    ...
    if ftype is not None:
        video_dict["ftype"] = ftype
    if FileNames is not None:
        video_dict["FileNames"] = FileNames
    if cal_frms is not None:
        video_dict["cal_frms"] = cal_frms
...
def PlayVideo_ext(video_dict, display_dict, Freezing, mt_cutoff, SIGMA=1):
    ...
    cap = cv2.VideoCapture(video_dict["fpath"])
    rate = int(
        1000 / display_dict["fps"]
    )  # duration each frame is present for, in milliseconds
    cap.set(cv2.CAP_PROP_POS_FRAMES, video_dict["start"] + display_dict["start"])
...
def Summarize(
    video_dict, Motion, Freezing, FreezeThresh, MinDuration, mt_cutoff, bin_dict=None
):
    """
    -------------------------------------------------------------------------------------
...
def Batch(
    ...
    for video_dict["file"] in video_dict["FileNames"]:
        ...
        try:
            ...
            NameError
        ):  # to be done for first file in list, before summary_all is created
            summary_all = summary
...
def Calibrate(video_dict, cal_pix, SIGMA, accept_p_frames=False):
    ...
    for x in range(1, video_dict["cal_frms"]):
        ...
        if ret:
            ...
            frame_pix_dif = np.absolute(
                frame_new[h_loc, w_loc] - frame_old[h_loc, w_loc]
            )
    ...
    hist.opts(
        title="Motion Cutoff: " + str(np.around(mt_cutoff, 1)),
